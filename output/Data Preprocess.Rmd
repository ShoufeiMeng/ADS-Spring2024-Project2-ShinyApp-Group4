...
server <- function(input, output, session) {
  ...
  data_filtered <- eventReactive(input$update, {
    tryCatch({
      req(input$state, input$year, input$category)  # Ensure inputs are not NULL
      # Your filtering logic here...
    }, error = function(e) {
      # Handle error, e.g., return an empty data frame or a special value
      print(e)  # Print the error for debugging
      data.frame()  # Return an empty data frame as a placeholder
    })
  })
  ...
  output$stats <- renderText({
    ...
    metric <- if(input$displayMetric == "frequency") "Disaster Count:" else "Total Project Cost:"
    value <- if(input$displayMetric == "frequency") {
      sum(data$frequency, na.rm = TRUE)
    } else {
      sum(data$TotalProjectCost, na.rm = TRUE)
    }
    paste(metric, formatC(value, format = "f", big.mark = ","))
  })
}
