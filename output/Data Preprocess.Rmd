  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("year", "Year",
                  ...
                  value = min(merged_data$Year), # set initial value to be the minimum year
                  step = 1),
...
server <- function(input, output) {
  output$map <- renderLeaflet({
    ...
    leaflet(data) %>%
      addTiles() %>%
      setView(lng = -98.5795, lat = 39.8283, zoom = 4) %>% # set default view
      addCircles(
        lng = ~longitude, lat = ~latitude, 
        radius = ~sqrt(frequency) * 1000, # Adjust point size according to the frequency
